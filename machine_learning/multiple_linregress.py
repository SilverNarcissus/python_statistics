...
class MultipleLinregress:
    ...
    def fit(array_x, array_y, alpha=0.05):
        ...
        residual_std_error = math.sqrt(abs(v_e))
        ...
        if significance_very_high:
            return MultipleLinregress(beta, r, "very high", interval, significance_conclusion)
        if significance_high:
            return MultipleLinregress(beta, r, "high", interval, significance_conclusion)
        return MultipleLinregress(beta, r, "no", interval, significance_conclusion)
...
result = MultipleLinregress.fit(array_x[:10], array_y[:10])
print(regression_evaluator(result.predict, array(array_x[10:15]), array(array_y[10:15])))
